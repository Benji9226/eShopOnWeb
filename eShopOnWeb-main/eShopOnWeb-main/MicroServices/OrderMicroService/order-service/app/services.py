...
# publish OrderCreated event (non-blocking) - consider outbox in prod
payload = {
"type": "OrderCreated",
"version": 1,
"data": {
...
q = await db.execute(select(models.Order).where(models.Order.buyer_id == buyer_id).order_by(models.Order.order_date.desc()))
